...
bool RecognizeCandle(string symbol, ENUM_TIMEFRAMES period, datetime time, int aver_period, CANDLE_STRUCTURE &res)
{
   ...
   if(aver < res.close) res.trend = UP_TREND;
   if(aver > res.close) res.trend = DOWN_TREND;
   if(aver == res.close) res.trend = SIDEWAY_TREND;
//--- กำหนกรูปแบบของ bull
   res.bull = res.open < res.close;
//---หาขนาดbody แท่งเทียน
...
//---ขนาดแท่งเทียนทั้งหมดรวมไส้
   double HL = res.high - res.low;
//--หาค่าเฉลี่ยของขนาดbodyแท่งเทียนก่อนหน้า
   double sum = 0;
